...
- `lib_path_template`: Custom template for library path generation (optional)
  - Template variables: `{{.Arch}}`, `{{.ArchTriple}}`, `{{.Version}}`, `{{.ArchVersion}}`
- `enable_lib_path`: Enable automatic library path templates (default: `False`)
  - Uses default templates for Debian multiarch and RedHat lib64 paths
...
# Results in runtime path evaluation using default templates:
# - /usr/lib/x86_64-linux-gnu/glibc-hwcaps/{{.Version}}  (Debian)
# - /usr/lib64/glibc-hwcaps/{{.ArchVersion}}              (RedHat)
# - /opt/{{.Arch}}/lib                                     (Custom)
```
...
# Template is evaluated at runtime:
# - For v1 binary: /opt/glibc-v1/lib (if it exists)
# - For v3 binary: /opt/glibc-v3/lib (if it exists)
```
